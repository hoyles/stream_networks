...
str(nz_waitaki_longfin_eel_downstream$observations)
table(nz_waitaki_longfin_eel_downstream$observations$data_type)

lendat <- read.csv("~/NZ_stream_networks/stream_networks/NZ/data/longfin_length_data.csv")
a <- lendat[,c(3:5,20:28)]
lens <- gather(a, key=whichlen, value=len, length1:length8)
rm(a)
lens <- lens[!is.na(lens$len),]
lens <- lens[!is.na(lens$nzsegment),]

# link to network
pos <- match(lens$nzsegment, network$nzsegment)
lens <- lens[!is.na(pos),]
pos <- pos[!is.na(pos)]
lens <- cbind(lens, network[pos,c("parent_s","child_s","dist_s","lat", "long")])
lens <- rename(lens, parent_i = parent_s, child_i = child_s, dist_i = dist_s)
...
p1 <- ggplot() +
  ...
  geom_point(data = lens, aes(x = long, y = lat), pch=19, cex=2, color="red", alpha=0.8) +
  xlab("Longitude") + ylab("Latitude") + 
  mytheme()
# ggsave(file.path(fig_len_dir, "Waitaki.png"), p1)

p2 <- ggplot() +
  geom_point(data = network, aes(x = long, y = lat), pch=19, cex=0.01, alpha=0.2) +
  geom_point(data = lens, aes(x = long, y = lat, color = "red"), pch=19, cex=1.5,alpha=0.8) +
  scale_color_brewer(palette = "Set1") +
  xlab("Longitude") + ylab("Latitude") + 
  guides(color=guide_legend(title="Encounter")) +
  facet_wrap(~year) +
  mytheme()
# ggsave(file.path(fig_len_dir, "Waitaki_byYear.png"), p2)
...
# }
# png(file.path(fig_len_dir, "Habitat_pairs.png"), width=15, height=10, units='in', res=200)
...
#   mytheme()
#   ggsave(file.path(fig_len_dir, paste0("Habitat_covariate_", covar_toUse[i],".png")),p)
# }

##################################
## model - encounter observations
##################################
nz_len_dir <- file.path(nz_dir, "waitaki_lengths_DownstreamOnly_VesselEffect")
dir.create(nz_len_dir, showWarnings=FALSE)
setwd(nz_len_dir)
AIC_list <- NULL

saveRDS(lens, file.path(nz_len_dir, "waitaki_lengths.rds"))
saveRDS(network, file.path(nz_len_dir, "network.rds"))
# saveRDS(hab_toUse, file.path(nz_len_dir, "habitat.rds"))
...
##### setup data frame
Data_Geostat <- data.frame( "Catch_KG" = lens$len, 
              "Year" = lens$year,
               "Vessel" = lens$fishmeth, 
               "AreaSwept_km2" = lens$dist_i, 
               "Lat" = lens$lat, 
               "Lon" = lens$long, 
               "Pass" = 0,
...
Extrapolation_List = StreamUtils::make_extrapolation_info( Region="Stream", 
  stream_info=cbind("Lat"=lens$lat, 
                    "Lon"=lens$long,
                    "child_i"=lens$child_i,
                    "Area_km2"=lens$dist_i), 
...
########################
temp_dir <- file.path(nz_len_dir, "temporal_gamma_deltalink")
...
plot_network(Spatial_List=Spatial_List, Extrapolation_List=Extrapolation_List, TmbData=Data, Data_Geostat=Data_Geostat, observations=TRUE, arrows=TRUE, root=FALSE, savedir=fig_len_dir, cex=0.2)
...
##################
space_dir <- file.path(nz_len_dir, "spatial_temporal_gamma_deltalink")
...
TmbList1 = make_model("TmbData"=Data, "Version"=Version, "RhoConfig"=RhoConfig, "loc_x"=Spatial_List$loc_x, "Method"=Method)
# Fixing the parameter
Map_new = TmbList1$Map
Map_new$logkappa1 <- Map_new$logkappa2
Map_new$L_omega1_z <- Map_new$logkappa2
TmbList = make_model("TmbData"=Data, "Map" = Map_new, "Version"=Version, "RhoConfig"=RhoConfig, "loc_x"=Spatial_List$loc_x, "Method"=Method)

Obj = TmbList[["Obj"]]
Obj$par[grep("logkappa",names(Obj$par))] = log(1/median(Network_sz[,'dist_s']))/10000
Obj$par[grep("L_omega1_z",names(Obj$par))] = 1
...
##################
space_dir <- file.path(nz_len_dir, "spatial_temporal_lognormal_deltalink")
dir.create(space_dir)
setwd(space_dir)
...
########################
st_dir <- file.path(nz_len_dir, "spatiotemporal_spatial_gamma_deltalink")
dir.create(st_dir)
setwd(st_dir)
...
## SPATIOTEMPORAL GAMMA DELTA LINK
########################
st_dir <- file.path(nz_len_dir, "spatiotemporal_spatial_lognormal_deltalink")
dir.create(st_dir)
setwd(st_dir)
...
##################
space_dir <- file.path(nz_len_dir, "spatial_temporal_lognormal_habitat")
dir.create(space_dir)
setwd(space_dir)
...
#####################
## spatiotemporal
st_dir <- file.path(nz_len_dir, "spatiotemporal_spatial_lognormal_habitat")
dir.create(st_dir)
setwd(st_dir)
...
AIC <- lapply(1:length(models), function(x){
  res <- readRDS(file.path(nz_len_dir, models[x], "Save.rds"))
